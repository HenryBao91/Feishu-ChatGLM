...
from handler.message_handler import OpenaiMessageEventHandler, ChatglmMessageEventHandler
...
if app_config.LLM == 'chatGLM':
	message_handler = ChatglmMessageEventHandler(app_config, feishu_conf)
else:
    message_handler = OpenaiMessageEventHandler(app_config, feishu_conf)
...
def route_im_message(ctx: Context, conf: Config, event: MessageReceiveEvent) -> Any:
    ...
    if "text" in json_content and json_content["text"].startswith("/"):
        if command_handler.handle_message(event):
            print("This a command message")
            mark_event_processed(event)
    else:
        app_logger.info("This is a text message")

        mentions = getattr(event.event.message, 'mentions', None)
        ...
        if chat_type == "group":
            if mentions is not None:
                try:
                    mentions_list = list(mentions)[0]
                    # print(f"Mentions list: {mentions_list}")
                    bot_mentioned = getattr(mentions_list, 'name', None) == app_config.BOT_NAME  #'ChatBotter'
                except Exception as e:
                    app_logger.error(f"Error processing mentions: {e}")
                    bot_mentioned = False
        elif chat_type == "p2p":
            bot_mentioned = True
        else:
