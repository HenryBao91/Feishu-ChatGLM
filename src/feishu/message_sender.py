...
class MessageSender:
    ...
    def send_text_message(self, user_id, msg, chat_id="", append=True):
        body = {
            "user_id": user_id,
            # "chat_id": "oc_61db870ebe4d2bd8f67f4e83bcfe8770",
            "msg_type": "text",
            "content": {
                "text": msg
            }
        ...
        req = Request('/open-apis/message/v4/send',
                      'POST',
        ...
        # if test main function, comment this code
        if resp.code == 0:
    ...
    def send_textX_message(self, receive_id, msg, receive_id_type="chat_id", append=True):
        body = {
        ...
        if resp.code != 0:
            app_logger.error("Failed to send message: %s", resp.msg)
            return None
...
if __name__ == '__main__':
    try:
        ...
        # deprecated API
        # message_sender.send_text_message("6215f231", "Hello BoBo Group")
        # new API
        message_id = message_sender.send_textX_message("oc_61db870ebe4d2bd8f67f4e83bcfe8770", "Hello BoBo Group", "chat_id")
        # reply API
